...
def visualize_sensitivity_invariance(skip_autograd=False, autograd_only=False):
    # --- 1. Configuration ---
    ...
    autograd_active = not skip_autograd
...
def parse_args():
    parser = argparse.ArgumentParser(description="PGF Sensitivity Invariance")
    parser.add_argument("--skip-autograd", action="store_true", help="Skip Autograd reference JVPs.")
    parser.add_argument("--autograd-only", action="store_true", help="Only run Autograd references (no PGF).")
    return parser.parse_args()


if __name__ == "__main__":
    args = parse_args()
    visualize_sensitivity_invariance(skip_autograd=args.skip_autograd, autograd_only=args.autograd_only)
